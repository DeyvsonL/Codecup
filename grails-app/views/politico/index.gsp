		...
		<div id="list-politico" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="fotoUrl" title="${message(code: 'politico.fotoUrl.label', default: 'Foto Url')}" />
					
						<g:sortableColumn property="qualidade" title="${message(code: 'politico.qualidade.label', default: 'Qualidade')}" />
				...
				<g:each in="${politicoInstanceList}" status="i" var="politicoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${politicoInstance.id}">${fieldValue(bean: politicoInstance, field: "nome")}</g:link></td>
					
						<td>${fieldValue(bean: politicoInstance, field: "descricao")}</td>
					
						<td>${fieldValue(bean: politicoInstance, field: "fotoUrl")}</td>
					
						<td>${fieldValue(bean: politicoInstance, field: "qualidade")}</td>
